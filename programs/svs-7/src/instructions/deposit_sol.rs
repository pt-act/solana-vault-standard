//! deposit_sol: deposit native SOL (lamports) into the vault.
...
pub fn handler(ctx: Context<DepositSol>, assets: u64, min_shares_out: u64) -> Result<()> {
    ...
    // Ensure wSOL amount is synced before using it in conversions.
    invoke(
    ...
    // Determine total assets by balance model
    ...
    let net_shares = {
        ...
        result.net_shares
        // NOTE: fee_shares are not minted here (see other variants).
    };
    ...
    // Transfer lamports into the wSOL token account.
    invoke(
    ...
    // Sync native to update token amount.
    invoke(
        &spl_token::instruction::sync_native(
            &ctx.accounts.token_program.key(),
            &ctx.accounts.wsol_vault.key(),
    ...
    // Update stored total_assets if needed

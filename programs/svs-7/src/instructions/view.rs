...
fn total_assets_for_view(vault: &SolVault, wsol_amount: u64) -> u64 {
...
pub fn preview_deposit(ctx: Context<VaultView>, assets: u64) -> Result<()> {
    ...
    let total_assets = total_assets_for_view(vault, ctx.accounts.wsol_vault.amount);
...
pub fn get_total_assets(ctx: Context<VaultView>) -> Result<()> {
    let total_assets = total_assets_for_view(&ctx.accounts.vault, ctx.accounts.wsol_vault.amount);
